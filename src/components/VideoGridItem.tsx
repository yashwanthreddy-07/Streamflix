...
const VideoGridItem = ({id,title,channel,views,postedAt,duration,thumbnailUrl,videoUrl}:VideoGridItemProps) => {
  ...
  return (
    <div className="flex flex-col gap-2" onMouseEnter={()=>setVideoPlaying(true)
        ...
        <div className="absolute bottom-1 right-1 bg-secondary-dark text-secondary text-sm px-0.5 rounded">
                ...
                <video className={`block h-full object-cover absolute inset-0 transition-opacity duration-200 delay-200 ${isVideoPlaying? "opacity-100": "opacity-0"} `} ref={VideoRef} muted playsInline src={videoUrl} />  
