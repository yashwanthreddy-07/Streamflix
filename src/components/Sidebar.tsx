    ...
    <aside   className={`w-56 lg:sticky absolute top-0 overflow-y-auto scrollbar-hidden pb-4 flex-col gap-2 px-2 ${
      ...
      >
        <div className="lg:hidden pt-2 pb-4 px-2 sticky top-0 bg-white">
          <div className='lg:hidden pt-2 pb-4 px-2 sticky top-0 bg-white'><HeaderElement /></div>
        ...
        <LargeSidebarSection visibleItemCount={5}>
         ...
         {playlists.map(playlist=>(
            <LargeSidebarItem key={playlist.id} title={playlist.name} IconOrImgUrl={ListVideo}  url={`/playlist?list=${playlist.id}`}/>
         ))}
         <hr/>
         </LargeSidebarSection>
         <hr/>
...
function LargeSidebarSection({children,title,visibleItemCount=Number.POSITIVE_INFINITY}:LargeSidebarSectionProps){
    const [isExpanded,setIsExpanded] = useState(false)
        ...
        childrenArray:childrenArray.slice(0,visibleItemCount)
        const ButtonIcon = isExpanded?ChevronUp:ChevronDown
        return (
            <div>
            {title && <div className='ml-4 mt-2 text-lg mb-1'>{title}</div>}
                    {visibleChildren}
                    {showExpandButton && (<Button onClick={()=>setIsExpanded(e=>!e)} variant="ghost" className='w-full flex items-centerrounded-lg gap-4 p-3
                    ' >
